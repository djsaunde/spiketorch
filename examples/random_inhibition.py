...
class SNN:
	...
	def __init__(self, seed=0, mode='train', n_input=784, n_exc=100, n_inh=25, n_inh_synapse=10, n_examples=(10000, 10000), dt=1, 
										lrs=(1e-4, 1e-2), c_inhib=17.4, sim_times=(350, 150, 350, 150), stdp_times=(20, 20),
																	update_interval=100, wmax=1.0, gpu=True, intensity=1):
		'''
		Constructs the spiking neural network.
		...
		if mode == 'train':
			...
			synapses = np.zeros([neurons.size, n_inh])
			synapses[np.arange(neurons.size), neurons] = 1

			if gpu:
				self.W['Ae_Ai'] = 22.5 * torch.from_numpy(synapses).cuda().float()
			else:
				self.W['Ae_Ai'] = 22.5 * torch.from_numpy(synapses).float()

			ordered = np.array([0] * (n_exc - n_inh_synapse) + [1] * n_inh_synapse)
			synapses = np.array([ np.random.permutation(ordered) for _ in range(n_inh) ])

			if gpu:
				self.W['Ai_Ae'] = c_inhib * torch.from_numpy(synapses).cuda().float()
			else:
				self.W['Ai_Ae'] = c_inhib * torch.from_numpy(synapses).float()
